...
import ir.alisalimik.kapstone.api.initializeCapstone
...
class CapstoneTests {
  ...
  fun testArm64Branch() = runTest {
    ...
    CapstoneEngine.build(Architecture.ARM64, Mode.LITTLE_ENDIAN) { detail = true }
        .use { cs ->
          val result = cs.disassemble(code, address = 0x1000)
          val insns = result.getOrThrow()
  ...
  fun testArm64MultipleInstructions() = runTest {
    ...
    val code =
        byteArrayOf(
            ...
            0x43,
            0x00,
            0xD1.toByte(), // sub sp, sp, #0x10
            0xE0.toByte(),
            0x03,
            0x00,
            0xF9.toByte(), // str x0, [sp]
            0xE0.toByte(),
            0x03,
            0x40,
            0xF9.toByte(), // ldr x0, [sp]
            0xFF.toByte(),
            0x43,
            0x00,
            0x91.toByte(), // add sp, sp, #0x10
            0xC0.toByte(),
            0x03,
            0x5F,
            0xD6.toByte() // ret
  ...
  fun testArmBranch() = runTest {
    ...
    CapstoneEngine.build(Architecture.ARM, Mode.ARM) { detail = true }
        .use { cs ->
          val result = cs.disassemble(code, address = 0x1000)
          val insns = result.getOrThrow()
  ...
  fun testArmMultipleInstructions() = runTest {
    ...
    val code =
        byteArrayOf(
            ...
            0x40,
            0x2D,
            0xE9.toByte(), // push {r4, lr}
            0x01,
            0x00,
            0xA0.toByte(),
            0xE1.toByte(), // mov r0, r1
            0x10,
            0x80.toByte(),
            0xBD.toByte(),
            0xE8.toByte() // pop {r4, pc}
            )
  ...
  fun testX86_64Call() = runTest {
    ...
    CapstoneEngine.build(Architecture.X86, Mode.MODE_64) { detail = true }
        .use { cs ->
          val result = cs.disassemble(code, address = 0x1000)
          val insns = result.getOrThrow()
