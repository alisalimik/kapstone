 ...
 *
 * This class provides a zero-cost abstraction over integer bit operations while maintaining
 * type safety through Kotlin's inline value classes. It's designed to work with enum classes
 * that represent bit flags, ensuring compile-time safety and runtime efficiency.
 *
 * **Key Features:**
...
value class BitField<T>(internal val value: ULong = 0UL) where T : Enum<T> {
    ...
    companion object {
        ...
        fun <T> of(vararg flags: T): BitField<T> where T : Enum<T> {
            return flags.fold(empty<T>()) { acc, flag ->
                acc.setFlag(flag)
            }
        }
    ...
    /**
     * Sets the specified flag in this bit field.
     * Returns a new BitField with the flag set.
     *
     * @param flag The flag to set
     * @return New BitField with the flag set
     */
    fun setFlag(flag: T): BitField<T> {
        val flagValue = (flag as Enum<*>).ordinal
        return BitField(value or (1UL shl flagValue))
    }
    ...
    fun hasFlag(flag: T): Boolean {
        val flagValue = (flag as Enum<*>).ordinal
        return (value and (1UL shl flagValue)) != 0UL
    }
    ...
    /**
     * Clears the specified flag from this bit field.
     * Returns a new BitField with the flag cleared.
     *
    ...
    fun clearFlag(flag: T): BitField<T> {
        val flagValue = (flag as Enum<*>).ordinal
        return BitField(value and (1UL shl flagValue).inv())
    }
    ...
    fun toggleFlag(flag: T): BitField<T> {
        val flagValue = (flag as Enum<*>).ordinal
        return BitField(value xor (1UL shl flagValue))
    }
    ...
    /**
     * Returns the complement of this bit field (logical NOT).
     * Note: This inverts all bits, which may include more than just the enum values.
     *
    ...
    infix fun and(flag: T): BitField<T> = if (hasFlag(flag)) BitField((flag as Enum<*>).ordinal.let { 1UL shl it }) else empty()
    ...
    /**
     * Converts this BitField to Int.
     * Note: This may truncate the value if it doesn't fit in 32 bits.
     *
    ...
    /**
     * Converts this BitField to UInt.
     * Note: This may truncate the value if it doesn't fit in 32 bits.
     *
