 ...
 */
fun <T : Enum<T>> Collection<T>.toBitField(): BitField<T> = BitField.Companion.fromFlags(this)

/**
 * Creates a BitField from an array of enum values.
 */
fun <T : Enum<T>> Array<T>.toBitField(): BitField<T> = BitField.Companion.fromFlags(this.toList())
